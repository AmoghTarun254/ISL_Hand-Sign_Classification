...
#Amogh: This function is redundant as it is not called in the main execution cycle of the code
...
def process_directory (main_folder_path):
    ...
    try:
        for char_folder in os.listdir(main_folder_path):
            char_folder_path = os.path.join(main_folder_path, char_folder) #Path to each folder inside the directory
...
def process_folder(char_folder_path):
    ...
    options_local = vision.HandLandmarkerOptions(base_options=base_options_local, min_hand_detection_confidence=0.5,
                                                 min_hand_presence_confidence=0.5, min_tracking_confidence=0.5,
                                                 num_hands=2)
...
if __name__ == "__main__":
    ...
    #process_folder(char_folder_path) #Amogh: char_folder_path contains both single and two-handed gesture images 